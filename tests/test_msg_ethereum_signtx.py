...
class TestMsgEthereumSigntx(common.KeepKeyTest):
    ...
    def test_ethereum_eip_1559_multibyte_chain_id(self):
        ...
        Regression for the multi-byte chain_id bug: the EIP-1559 hash step used
        hash_rlp_field((uint8_t*)&chain_id, 1), which on little-endian ARM fed
        only the least-significant byte into keccak. For Base (8453 = 0x2105)
        that hashed 0x05, so the signature recovered to an unrelated address
        with no funds. The RLP *length* was computed correctly from the full
        value, and the legacy EIP-155 path was always correct — only the
        EIP-1559 hash was wrong. Affected: Base (8453), Arbitrum (42161),
        Avalanche (43114). Unaffected: ETH (1), OP (10), BSC (56), Polygon (137).

        Asserting a golden r/s would need a device run to produce it. Instead
        sign one identical transaction under two chain ids chosen so the BUGGY
        firmware cannot tell them apart:
        ...
        Vault cannot reach this path: it forces legacy txs for chainId >= 256
        (eip1559Ok = chainId < 256). The BEX builds type-2 with no such guard,
        which is why the failures only ever showed up there.
        """
        self.requires_fullFeature()
