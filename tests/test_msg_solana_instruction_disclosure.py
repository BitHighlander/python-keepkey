...
class TestSolanaInstructionDisclosure(common.KeepKeyTest):
    ...
    def test_all_verified_instruction_fields_change_oled_review(self):
        ...
        cases.extend([
            ...
            ("token-mint-to-mint",
             one(TOKEN_PROGRAM, [signer, key(44), key(45)], [1, 2, 0],
                 bytes([7]) + u64(1)),
             one(TOKEN_PROGRAM, [signer, key(46), key(45)], [1, 2, 0],
                 bytes([7]) + u64(1))),
            ("token-mint-to-destination",
             one(TOKEN_PROGRAM, [signer, key(44), key(45)], [1, 2, 0],
                 bytes([7]) + u64(1)),
             one(TOKEN_PROGRAM, [signer, key(44), key(47)], [1, 2, 0],
                 bytes([7]) + u64(1))),
            ("token-burn-mint",
             one(TOKEN_PROGRAM, [signer, key(48), key(49)], [1, 2, 0],
                 bytes([8]) + u64(1)),
             one(TOKEN_PROGRAM, [signer, key(48), key(50)], [1, 2, 0],
                 bytes([8]) + u64(1))),
            ("token-burn-source",
             one(TOKEN_PROGRAM, [signer, key(48), key(49)], [1, 2, 0],
                 bytes([8]) + u64(1)),
             one(TOKEN_PROGRAM, [signer, key(51), key(49)], [1, 2, 0],
                 bytes([8]) + u64(1))),
