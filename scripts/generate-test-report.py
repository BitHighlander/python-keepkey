...
SECTIONS = [
    ...
    ('Z', 'Zcash Orchard Clear-Signing', '7.15.0',
     'Shielded Orchard transactions via PCZT streaming. Firmware verifies ZIP-244 header, '
     'transparent, and Orchard digests from plaintext metadata; recomputes Orchard cmx from '
     'recipient/value/rseed before display; signs only after privacy outputs and final fee are confirmed.',
     [
         'FVK: Derive ak, nk, rivk components via ZIP-32 Orchard path',
         'PCZT: Stream verified header, transparent data, and Orchard actions before signatures',
         'DISPLAY: Transparent recipients, Orchard-only UA outputs, amounts, and final fee',
         'REJECT: Legacy host sighashes, digest mismatches, Sapling, and cmx metadata tampering',
     ],
     [
         ...
         ('Z5', 'test_msg_zcash_sign_pczt', 'test_rejects_legacy_host_sighash',
          'Reject legacy Orchard sighash', 'Host-provided per-action sighashes are refused.', []),
         ('Z6', 'test_msg_zcash_sign_pczt', 'test_multi_action_device_sighash',
          'Sign multiple Orchard actions',
          'Device computes the ZIP-244 sighash and shows each shielded privacy output plus fee.',
          ['Orchard output', 'Fee confirm']),
         ('Z7', 'test_msg_zcash_sign_pczt', 'test_rejects_header_digest_mismatch',
          'Reject bad header digest', 'Plaintext header fields must match header_digest.', []),
         ('Z8', 'test_msg_zcash_sign_pczt', 'test_rejects_sapling_digest',
          'Reject Sapling component', 'Sapling digest is outside this signing scope and is refused.', []),
         ('Z9', 'test_msg_zcash_sign_pczt', 'test_rejects_missing_action_digest_fields',
          'Reject incomplete Orchard action', 'Every action must include fields covered by orchard_digest.', []),
         ('Z10', 'test_msg_zcash_sign_pczt', 'test_rejects_orchard_recipient_mismatch',
          'Reject recipient cmx mismatch', 'Displayed Orchard receiver must recompute to action cmx.', []),
         ('Z11', 'test_msg_zcash_sign_pczt', 'test_rejects_orchard_value_mismatch',
          'Reject value cmx mismatch', 'Displayed Orchard amount must recompute to action cmx.', []),
         ('Z12', 'test_msg_zcash_sign_pczt', 'test_signatures_are_64_bytes',
          'Signature format', 'Orchard signatures must be exactly 64 bytes (RedPallas).',
          ['Orchard output', 'Fee confirm']),
         ('Z13', 'test_msg_zcash_sign_pczt', 'test_different_accounts_different_signatures',
          'Different accounts', 'Same clear-signed request produces account-specific signatures.', []),
         ('Z14', 'test_msg_zcash_sign_pczt', 'test_transparent_shielding_single_input',
          'Transparent to shielded',
          'Transparent output is displayed, transparent digest is verified, then Orchard output is signed.',
          ['Transparent output', 'Orchard output', 'Fee confirm']),
         ('Z15', 'test_msg_zcash_sign_pczt', 'test_transparent_shielding_multiple_inputs',
          'Multi-input shielding', 'Multiple transparent inputs feed one Orchard output with device sighashes.',
          ['Orchard output', 'Fee confirm']),
         ('Z16', 'test_msg_zcash_sign_pczt', 'test_rejects_host_provided_transparent_sighash',
          'Reject transparent host sighash', 'Transparent input sighashes must be computed on device.', []),
         ('Z17', 'test_msg_zcash_sign_pczt', 'test_rejects_transparent_digest_mismatch',
          'Reject transparent digest mismatch', 'Streamed transparent plaintext must match transparent_digest.', []),
     ]),
