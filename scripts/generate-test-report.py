...
FULL_SEQUENCE_TESTS = {
    ...
    # ERC-7730 runtime: the order of the screens is the claim.
    ('test_msg_ethereum_erc7730_runtime', 'test_interpolated_intent_parts_show_the_same_values_as_fields'),
    ('test_msg_ethereum_erc7730_runtime', 'test_embedded_call_is_shown_under_a_blind_sign_warning'),
    ('test_msg_ethereum_erc7730_runtime', 'test_inner_call_is_clear_signed_with_its_own_definition'),
    ('test_msg_ethereum_erc7730_runtime', 'test_an_inner_definition_the_device_refuses_falls_back_to_blind'),
...
SECTIONS = [
    ...
    ('K', 'Seed Generation Hardening (7.15.0+)', '7.15.0',
    ...
    ('EX', 'ERC-7730 Runtime - What 7.15 Shows For A Certified Definition', '7.15.0',
     'A host may send a signed ERC-7730 definition before an Ethereum transaction. The device '
     'verifies it while it streams and then shows the transaction through it: the contract action, '
     'the signer\'s labels beside values the device decodes from the signed calldata, and inner '
     'calls reviewed with their own definition. The definition only adds screens. The '
     'ordinary review and signature follow unchanged: every test here that signs first signs '
     'the same transaction on the ordinary path and requires an identical signature.\n'
     'In 7.15 this runs only with AdvancedMode on, and the definition\'s signer is shown as a runtime '
     'signer, "NOT verified by KeepKey". It is annotation, not KeepKey-verified clear signing.',
     ['WHAT THE DEVICE EXECUTES (one capability table, shared by the preload verifier, the',
      'runtime and python-keepkey\'s compiler; anything else is refused at preload, before',
      'any screen):',
      '- Formatters: raw, amount, tokenAmount, nftName, date, duration, unit, enum,',
      '  addressName, embedded calldata. Containers @.from/@.to/@.value. A signed',
      '  constant is shown only as a raw field, as the signer\'s own.',
      '- Display: intent; interpolated intent as numbered parts; fields; groups; one array',
      '  iterated at a time. Condition "optional" only, and it always shows.',
      '- Refused: slices, packed words, never/ifIn/ifNotIn/mustMatch/ifEmpty, nested',
      '  iteration, ABIs deeper than 8. The compiler omits visible:never fields.',
      '- Official registry (pinned 9f37816a): 1,326 of 1,450 calldata formats pass the',
      '  device\'s preload checks (asserted in lockstep by test_erc7730_compiler).',
      '',
      'WHERE EACH FACT COMES FROM:',
      '- Values: decoded on device from the signed calldata, replayed and hash-checked on',
      '  every pass. Token tickers and decimals: the firmware table only. Signer text',
      '  (labels, messages, units, enum labels) is escaped and shown beside the raw value.',
      '',
      'EMBEDDED CALLS (7.15 rule: what cannot be clear-signed is shown under a',
      '"Blind signature" warning; 7.16 will reject it):',
      '- The inner definition is requested by chain, callee and selector read from the',
      '  signed calldata, and bound before any inner screen. One level deep.',
      '- Blind in 7.15: no inner definition; a refused one; one showing @.value with no',
      '  amountPath; depth 2; calls inside an array; inner bytes without a selector.',
      '',
      'KNOWN LIMITS (fail closed, no signature): parallel arrays pair by index and a',
      'shorter one aborts mid-review; a fixed index into a short dynamic array aborts; inner',
      'bytes not canonical for the inner ABI abort after the outer screens. The board pager',
      'may wrap a checksummed address across two OLED pages; every page must be confirmed.',
      'Open for 7.16: Safe DELEGATECALL is shown only as the operation field.'],
     [
         ('EX1', 'test_msg_ethereum_erc7730_runtime',
          'test_program_outside_capability_table_is_refused_at_preload',
          'Unexecutable programs are refused at preload',
          "A definition using a formatter, condition or path step outside the runtime's capability table is refused while it streams, before any screen. A review can never start and then fail part-way.",
          []),
         ('EX2', 'test_msg_ethereum_erc7730_runtime',
          'test_path_outside_abi_is_refused_at_preload',
          'Paths outside the ABI are refused at preload',
          "Every value path is walked against the function's ABI at preload: a tuple index out of range, a step into a scalar or a path ending on an array is refused before any screen.",
          []),
         ('EX3', 'test_msg_ethereum_erc7730_runtime',
          'test_raw_field_screens_show_exact_text',
          'Raw fields show the exact decoded value',
          "Each field is its own required confirmation under a device-owned title; the label is the signer's, the value is decoded from the signed calldata.",
          ['Signer field with decoded value']),
         ('EX4', 'test_msg_ethereum_erc7730_runtime',
          'test_token_amount_uses_the_firmware_token_table',
          'Token amounts use the firmware token table',
          "Ticker and decimals come only from the firmware's own table for the chain, rendered exactly as the ordinary Ethereum review renders them.",
          ['Amount with firmware ticker']),
         ('EX5', 'test_msg_ethereum_erc7730_runtime',
          'test_token_named_by_calldata_wins_over_the_hosts_claim',
          'The token is read from calldata, not the host',
          "The transaction goes to one contract; the calldata names another token. The device shows the calldata's token, and an unknown token as the exact integer plus its address.",
          ['Unknown token, exact integer and address']),
         ('EX6', 'test_msg_ethereum_erc7730_runtime',
          'test_signer_label_cannot_name_an_unknown_token',
          'A signer label cannot name a token',
          'A label saying "USDC amount" over an unknown token still shows the raw integer, "unknown token" and the address.',
          ['Signer label beside unknown token']),
         ('EX7', 'test_msg_ethereum_erc7730_runtime',
          'test_threshold_message_is_shown_beside_the_exact_amount',
          'Threshold messages sit beside the amount',
          'At or above the threshold the signer\'s message is shown, marked "Signer:", above the exact amount, never instead of it; below it no message appears.',
          ['Signer message above the exact amount']),
         ('EX8', 'test_msg_ethereum_erc7730_runtime',
          'test_native_alias_discloses_signer_mapping_and_token_address',
          'Native-currency aliases disclose the signer mapping and token address',
          "An address in the signer's alias set is marked as a signer native alias and shown in full before the native-unit amount; any other address stays an unknown token. The firmware token table wins over an alias.",
          ['Signer alias, token address and native-unit amount']),
         ('EX9', 'test_msg_ethereum_erc7730_runtime',
          'test_address_name_marks_only_the_signing_account',
          '"(this wallet)" marks only the signing account',
          'The signing address is derived on device; an address one byte away is shown without the mark. Addresses are EIP-55 and never truncated.',
          ['Address marked (this wallet)']),
         ('EX10', 'test_msg_ethereum_erc7730_runtime',
          'test_containers_and_signed_constants',
          'Transaction containers and signed constants',
          '@.to is read from the transaction being signed; a constant comes from the signed definition.',
          ['Container and constant fields']),
         ('EX11', 'test_msg_ethereum_erc7730_runtime',
          'test_interpolated_intent_parts_show_the_same_values_as_fields',
          'Interpolated intent as numbered parts',
          'The intent sentence is shown as numbered parts after the plain intent: "Intent text i of n" for the signer\'s fragments, "Intent value i of n" for values the device formats, identical to the matching field.',
          ['Intent text part', 'Intent value part', 'Matching field']),
         ('EX12', 'test_msg_ethereum_erc7730_runtime',
          'test_phase_c_formatters_show_exact_text',
          'Date, duration, unit and enum formatters',
          'Each value is formatted on device and always shown with its raw integer; a unit\'s base is preceded by "unit set by signer", so the mark is never on a later page than the value.',
          ['Unit value with raw integer']),
         ('EX13', 'test_msg_ethereum_erc7730_runtime',
          ...
          'A mapped value shows "label (value)" after "label set by signer"; an unmapped one shows "value (unmapped)".',
          ['Unmapped enum value']),
         ('EX14', 'test_msg_ethereum_erc7730_runtime',
          'test_nft_shows_the_collection_address',
          'NFTs show the token ID and collection address',
          'No collection name is claimed: the token ID and the full collection address.',
          ['Token ID and collection']),
         ('EX15', 'test_msg_ethereum_erc7730_runtime',
          'test_malformed_calldata_is_refused_before_a_constant_field',
          'Malformed calldata is refused before any field',
          'The up-front validation pass rejects calldata that does not match the ABI before the first field, even one showing a constant.',
          []),
         ('EX16', 'test_msg_ethereum_erc7730_runtime',
          'test_iteration_shows_every_element_numbered',
          'Arrays show every element, numbered',
          'Each element\'s fields are titled "Signer field i of N"; an empty array shows no element and still signs.',
          ['Element 1 of 2', 'Element 2 of 2']),
         ('EX17', 'test_msg_ethereum_erc7730_runtime',
          'test_grouped_tuple_iteration_and_optional_fields',
          'Grouped tuple arrays and optional fields',
          'Groups only group; an "optional" field is always shown. Nothing given to the device is hidden.',
          ['Tuple element fields', 'Optional field shown']),
         ('EX18', 'test_msg_ethereum_erc7730_runtime',
          'test_embedded_call_is_shown_under_a_blind_sign_warning',
          '7.15: embedded calls without a definition are blind',
          'A "Blind signature" screen, then the inner call\'s callee, selector, length, value and authority, all read from the signed calldata. 7.16 rejects instead.',
          ['Blind signature warning', 'Inner call summary']),
         ('EX19', 'test_msg_ethereum_erc7730_runtime',
          'test_inner_call_is_clear_signed_with_its_own_definition',
          'Inner calls are clear-signed with their own definition',
          'The device requests the inner definition by chain, callee and selector read from the signed calldata, binds it before any inner screen, runs it under "Inner" titles, then resumes the outer definition.',
          ['Call summary', 'Inner action', 'Inner fields', 'Outer field after the inner call']),
         ('EX20', 'test_msg_ethereum_erc7730_runtime',
          'test_inner_call_without_a_definition_is_blind_in_715',
          'No inner definition means a blind-sign warning',
          'When the host has no definition for the inner call, the 7.15 blind path follows; it is never a silent blind sign.',
          ['Blind signature warning']),
         ('EX21', 'test_msg_ethereum_erc7730_runtime',
          'test_inner_definition_for_another_call_is_refused',
          'An inner definition for another selector is refused',
          'A hostile host substitutes the inner definition; the binding check refuses it before any inner screen.',
          []),
         ('EX22', 'test_msg_ethereum_erc7730_runtime',
          'test_inner_bytes_changed_in_an_inner_pass_are_refused',
          'Changed inner bytes are refused',
          'Every inner pass replays the whole outer calldata against the reviewed digest; a change on a later pass is refused before any inner field.',
          []),
         ('EX23', 'test_msg_ethereum_erc7730_runtime',
          'test_only_a_raw_field_shows_a_signer_constant',
          'Only a raw field shows a signer constant',
          'A signer constant formatted as an amount (or any formatter but raw) would look like a decoded value, so preload refuses it; as a raw field it is shown as the signer\'s own field.',
          ['Constant as a raw field']),
         ('EX24', 'test_msg_ethereum_erc7730_runtime',
          'test_control_characters_in_a_value_are_escaped',
          'Control characters are escaped',
          'A newline or tab inside a value is shown as \\x0a / \\x09, so no value can fake a line break.',
          ['Escaped value']),
         ('EX25', 'test_msg_ethereum_erc7730_runtime',
          'test_a_raw_value_too_long_to_capture_is_shown_blind',
          'Over-long raw values are shown blind with their length',
          'A value longer than the device can hold gets a "Blind signature" screen and "Not shown: N bytes".',
          ['Blind signature warning', 'Length of the unshown value']),
         ...
         ('EX27', 'test_msg_ethereum_erc7730_runtime',
          ...
          'A refused inner definition falls back to blind',
          'An inner definition the device cannot execute, or signed by an unknown key (also one refused on a later chunk of a multi-chunk stream), is dropped and the call is shown blind; the outer review then continues.',
          ['Blind signature warning', 'Outer review continues']),
         ('EX28', 'test_msg_ethereum_erc7730_runtime',
          'test_inner_definition_for_another_callee_or_chain_is_refused',
          'Inner definitions are bound to callee and chain',
          'Substituting a definition for another contract or another chain is refused before any inner screen.',
          []),
         ('EX29', 'test_msg_ethereum_erc7730_runtime',
          'test_inner_calls_read_their_own_containers',
          'Inner calls read their own context',
          'Inside the inner call @.value is the value it moves, @.from whose authority it runs with and @.to the callee.',
          ['Inner value, authority and callee']),
         ('EX30', 'test_msg_ethereum_erc7730_runtime',
          'test_embedded_calls_inside_an_iteration_are_shown_blind',
          'Calls inside an array are shown blind',
          'Each element of a multicall gets its own blind-sign warning and summary (multicall clear-signing is future work).',
          ['Blind warning per element']),
         ('EX31', 'test_msg_ethereum_erc7730_runtime',
          'test_a_fixed_index_into_a_short_array_fails_closed',
          'A fixed index past a short array fails closed',
          "Preload cannot know a dynamic array's length; reaching path.[0] of an empty array aborts without a signature.",
          []),
         ('EX32', 'test_msg_ethereum_erc7730_runtime',
          'test_an_inner_value_the_calldata_does_not_carry_is_never_shown',
          'An inner value the calldata does not carry is never shown',
          'Without amountPath the calldata does not say what the inner call moves; an inner definition that shows @.value is refused and the call shown blind.',
          ['Blind signature warning']),
         ('EX33', 'test_msg_ethereum_erc7730_runtime',
          'test_parallel_arrays_pair_by_index_and_a_short_one_fails_closed',
          'Parallel arrays pair by index',
          'amounts[i] is shown with tokens[i]; a shorter second array aborts without a signature.',
          ['Paired element']),
         ('EX34', 'test_msg_ethereum_erc7730_runtime',
         ...
         ('EX36', 'test_msg_ethereum_erc7730_runtime',
          ...
          []),
         ('EX37', 'test_msg_ethereum_erc7730_runtime',
          'test_a_long_typed_data_value_points_to_the_walk_not_blind',
          'A long typed-data value is not a blind sign',
          'The typed-data walk shows every leaf in full; a raw field too long to capture says "Shown above in full: N bytes" with no blind-sign warning, and the signature equals the ordinary one.',
          ['Shown above in full']),
         ('EX38', 'test_msg_ethereum_erc7730_runtime',
          'test_a_refused_inner_call_does_not_blind_the_next_one',
          'A refused inner call does not blind the next one',
          'With two embedded calls, the first inner definition is refused and shown blind; the second is still clear-signed with its own definition.',
          ['Blind first call', 'Second call clear-signed']),
     ]),
...
MUST_RUN_MODULES = {
    ...
    # Block 7b: every ERC-7730 runtime row in section EX is part of 7.15.
    'test_msg_ethereum_erc7730_runtime': '7.15.0',
}
